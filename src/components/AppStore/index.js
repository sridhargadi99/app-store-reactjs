...
class AppStore extends Component {
  state = {textId: tabsList[0].tabId, searchInput: ''}

  getAppDetails = () => {
    const {textId} = this.state
    return appsList.filter(eachApp => eachApp.category === textId)
  }

  getAppContent = (id, content) => {
    const appResult = appsList.filter(eachApp => {
      let getDetails
      if (id === eachApp.category) {
        getDetails = eachApp.appName
          .toLowerCase()
  ...
  byClickingTab = uniqueId => {
    this.setState({textId: uniqueId})
  }

  byAddingContent = event => {
    this.setState({searchInput: event.target.value})
  }

  render() {
    const {textId, searchInput} = this.state
    let finalAppDetails
    if (searchInput === '') {
      finalAppDetails = this.getAppDetails()
    } else {
      finalAppDetails = this.getAppContent(textId, searchInput)
    }
    return (
      <div className="main-container">
        <div className="app-store-container">
          <h1 className="main-heading-style">App Store</h1>
          ...
          <ul className="tab-list-container">
            {tabsList.map(eachList => (
              <TabItem
                byClickingTab={this.byClickingTab}
                listDetails={eachList}
                key={eachList.tabId}
                activeIdStatus={textId === eachList.tabId}
              />
            ))}
          </ul>

          <ul className="app-list-container">
            {finalAppDetails.map(eachApp => (
